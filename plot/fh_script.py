...
#Calculating mean velocity for ocean speed
...
#Inference results
def fh_inference(area, fh, u_vel, v_vel, title, step = 5, min_l = -0.5e-5, max_l = 1.44e-5, compare_norkyst_area = None):

    if compare_norkyst_area is None:
        #calculate means
        u_vel, v_vel = mean_inference(area)
        #calculate f/h contours
        fh = fh_inference(area)
        #plot
        fig, ax = plt.subplots(figsize = (10,12), subplot_kw={'projection' : ccrs.NorthPolarStereo()})
        step = step
        min_l = min_l
